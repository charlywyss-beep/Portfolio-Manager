                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-muted/30 sticky top-0 backdrop-blur-md z-10">
                            ...
                            <tr className="border-b border-border">
                                ...
                                <th className="text-right py-1 px-2 font-medium text-muted-foreground text-xs" style={{ width: '70px', marginLeft: '50px' }}>%</th>
                            ...
                            {sortedPositions.map((p) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Daily Performance */}
                                        <td className={cn("py-3 px-2 text-right font-medium text-xs", isDailyPositive ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400")} style={{ width: '70px' }}>
                                            {isDailyPositive ? '+' : ''}{p.dailyGainPercent.toFixed(2)}%
                                        </td>
                                        <td className={cn("py-3 px-2 text-right font-medium", isDailyPositive ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400")} style={{ width: '100px' }}>
                                            {isDailyPositive ? '+' : ''}{formatCurrency(dailyGainCHF, 'CHF').replace('CHF', '').trim()}
                                        </td>
                                        {/* Total Performance */}
                                        <td className={cn("py-3 px-2 text-right font-medium text-xs", isTotalPositive ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400")} style={{ width: '70px', marginLeft: '50px' }}>
                        ...
                        <tfoot className="bg-muted/20 border-t border-border">
                            <tr>
                                ...
                                <td className="py-3 px-2" style={{ width: '70px' }}></td>
                                ...
                                <td className="py-3 px-2" style={{ width: '70px', marginLeft: '50px' }}></td>
